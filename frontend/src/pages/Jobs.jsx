  ...
  <Card className="relative">
    <div className="flex items-start justify-between mb-3">
      <div className="flex-1 min-w-0 pr-3">
        <h3 className="text-base font-semibold text-stone-900 dark:text-stone-100 truncate">{job.title}</h3>
        <div className="flex items-center flex-wrap gap-3 text-xs text-stone-500 dark:text-stone-400 mt-1">
          {job.location && (
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <button
          ...
          className="p-1.5 hover:bg-stone-100 dark:hover:bg-stone-700 rounded-lg transition-colors ml-1"
        >
        ...
        <button
          ...
          className="p-1.5 hover:bg-red-50 dark:hover:bg-red-950/40 rounded-lg transition-colors"
        >
    ...
    {job.description && (
      <p className="text-sm text-stone-500 dark:text-stone-400 line-clamp-2 mb-3">{job.description}</p>
    )}

    {job.requiredSkills?.length > 0 && (
      <div className="mb-2">
        <p className="text-2xs font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-wide mb-1.5">Required</p>
        <div className="flex flex-wrap gap-1.5">
          {job.requiredSkills.slice(0, 6).map((skill, i) => (
            <Badge key={i} variant="danger" size="sm">{skill}</Badge>
          ))}
          {job.requiredSkills.length > 6 && (
            <span className="text-xs text-stone-400 dark:text-stone-500">+{job.requiredSkills.length - 6}</span>
          )}
        </div>
      </div>
    )}
    ...
    {job.operationalSkills?.length > 0 && (
      <div className="mb-2">
        <p className="text-2xs font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-wide mb-1.5">Operational</p>
        <div className="flex flex-wrap gap-1.5">
          {job.operationalSkills.slice(0, 3).map((skill, i) => (
            <Badge key={i} variant="info" size="sm">{skill}</Badge>
          ))}
          {job.operationalSkills.length > 3 && (
            <span className="text-xs text-stone-400 dark:text-stone-500">+{job.operationalSkills.length - 3}</span>
          )}
        </div>
      </div>
    )}
    ...
    {job.hygieneSkills?.length > 0 && (
      <div>
        <p className="text-2xs font-semibold text-stone-400 dark:text-stone-500 uppercase tracking-wide mb-1.5">Hygiene (+5% each)</p>
        <div className="flex flex-wrap gap-1.5">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <textarea
            ...
            className="w-full rounded-xl border border-stone-300 dark:border-stone-700 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition-colors resize-none"
